...
# --- CONTEÚDO PRINCIPAL (ABA GALPÃO / CLIMA / PRODUÇÃO INTEGRADOS) ---
with menu_tabs[0]:
    
    # Cartão Principal do Layout do Utilizador
    str.markdown("""
    ...
    # Controlo interativo da Fechadura e Alimentação
...
with menu_tabs[7]: # Aba IA Mestre
    ...
    if not api_key_env:
        ...
        str.markdown("""
        **Como corrigir agora:**
        1. Clica em **Manage App** (no canto inferior direito) -> **Settings** -> **Secrets**.
        2. Escreve exatamente:  
        ...
        if str.button("Executar Auditoria Geral com Gemini"):
            try:
                ...
                str.error(f"Erro ao chamar a IA. Verifique se a sua chave copiada está correta. Detalhes: {e}")
